...
prim_grid <- function(input, y, probs, threshold = 0, vdim = 2, ...) {
  ...
  for (i in seq_len(num_com)) {
    ...
    points_box_x <- lapply(1:num, function(s)
      model_prim$x[[s]] %>% as_tibble())
    ...
    interest_box <- sapply(1:num,
      function(s) points_box_y[[s]][points_box_y[[s]] > threshold])
...
prim_select <- function(input, y, probs, var, box, ...) {
  ...
  #Prim model
  #model_prim <- prim.box(x=var_x, y=y*probs, threshold.type = 1, threshold = 0)
  ...
  #Scenario features
  df <- data_frame(
    feature = c("fun", "mass", "wdens", "wcov"), value = 0)
